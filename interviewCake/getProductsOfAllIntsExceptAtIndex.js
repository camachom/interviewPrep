function getProductsOfAllIntsExceptAtIndex(intArray) {
  ...
  const productAllIntsBeforeIndex = [1];
  let preProduct = 1;
  intArray.forEach((int, i) => {
    preProduct *= int;
    productAllIntsBeforeIndex.push(preProduct);
  });

  const productAllIntsAfterIndex = [];
  let postProduct = 1;
  for (var i = intArray.length - 1; i >= 0; i--) {
    productAllIntsAfterIndex[i] = postProduct;
    postProduct *= intArray[i];
  }

  const products = [];
  for (var i = 0; i < productAllIntsAfterIndex.length; i++) {
    products.push(productAllIntsAfterIndex[i] * productAllIntsBeforeIndex[i]);
  }
