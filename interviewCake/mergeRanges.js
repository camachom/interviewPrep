function mergeRanges(meetings) {
  ...
  const result = sortedMeetings.reduce((acc, val, idx) => {
    if (idx === 0) {
      return acc;
    }

    if (val.startTime <= currentEnd) {
      currentStart = Math.min(val.startTime, currentStart);
      currentEnd = Math.max(val.endTime, currentEnd);
    } else {
      acc.push({ startTime: currentStart, endTime: currentEnd });
      currentStart = val.startTime;
      currentEnd = val.endTime;
    }
    return acc;
  }, []);

  result.push({ startTime: currentStart, endTime: currentEnd });
  return result;
}
